Promise.all([
  import('../../src/data/examCatalog/depth/aama-cma.mjs'),
  import('../../src/data/examCatalog/depth/danb-cda.mjs'),
  import('../../src/data/examCatalog/depth/certified-dietary-manager-cfpp.mjs'),
  import('../../src/data/examCatalog/depth/nasm-certified-nutrition-coach.mjs')
]).then(([a, b, c, d]) => {
  const files = [
    ['aama-cma', a.default],
    ['danb-cda', b.default],
    ['certified-dietary-manager-cfpp', c.default],
    ['nasm-certified-nutrition-coach', d.default]
  ];
  const check = (name, d) => {
    const issues = [];
    const sec = [];
    if (d.salaryOutlook) {
      ...
      if (!s.summary || s.summary.length < 250) issues.push('salary.summary<250 (' + (s.summary ? s.summary.length : 0) + ')');
    ...
    if (d.passRate) {
      ...
      if (!p.summary || p.summary.length < 250) issues.push('passRate.summary<250 (' + (p.summary ? p.summary.length : 0) + ')');
    ...
    if (d.resourceComparison && d.resourceComparison.rows && d.resourceComparison.rows.length) {
      ...
      d.resourceComparison.rows.forEach(r => { if (!Array.isArray(r.values) || !r.values.length) issues.push('resource row bad values: ' + r.label); });
    }
    ...
    console.log('=== ' + name + ' ===');
    console.log('sections (' + sec.length + '):', sec.join(', '));
    console.log('salary words:', d.salaryOutlook ? d.salaryOutlook.summary.split(/\s+/).length : 0,
      '| passRate words:', d.passRate ? d.passRate.summary.split(/\s+/).length : 0,
      '| salary rows:', (d.salaryOutlook && d.salaryOutlook.rows.length) || 0,
      '| passRate rows:', (d.passRate && d.passRate.rows.length) || 0,
      '| weeks:', (d.studyPlan && d.studyPlan.weeks.length) || 0,
      '| prep:', (d.prepStrategies && d.prepStrategies.items.length) || 0,
      ...
      '| examDay tl:', (d.examDay && d.examDay.timeline.length) || 0);
  ...
  let total = 0;
  files.forEach(([n, d]) => { total += check(n, d); });
  console.log('\nTOTAL ISSUES:', total);
